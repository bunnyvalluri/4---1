...
export function Sidebar({ userName = 'Candidate', userEmail = '' }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* ======================================================== */}
      <div className="lg:hidden w-full border-b border-slate-200/80 bg-white/95 backdrop-blur-md px-3 py-2 overflow-x-auto flex items-center gap-2 sticky top-14 sm:top-16 z-30 shadow-2xs [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        {mobileNavItems.map((item) => {
          ...
          return (
            <Link
              key={item.label}
              href={item.href}
              className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-xl shrink-0 transition-all active:scale-95 select-none ${
                isActive
