...
export function UserHeader({
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/user/recommendations?q=${encodeURIComponent(searchQuery.trim())}`);
  ...
  return (
    <header className="sticky top-0 z-30 w-full border-b border-slate-200/90 bg-white/95 backdrop-blur-md pt-[env(safe-area-inset-top,0px)] transition-all select-none">
      <div className="flex h-16 items-center justify-between px-3.5 sm:px-6 lg:px-8 max-w-7xl mx-auto gap-2 sm:gap-4">
        {/* Left: Mobile Menu Trigger + Brand Logo & Desktop Telemetry */}
        <div className="flex items-center gap-2.5 sm:gap-3 shrink-0">
          {onOpenMobileMenu && (
            <button
              type="button"
              onClick={onOpenMobileMenu}
              className="lg:hidden flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
          ...
          <Link
            ...
            aria-label="CareerAI User Portal Home"
          >
        ...
        >
          <div className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Search careers, skills, roadmaps... (Enter)"
        ...
        {/* Right: Mobile Search Button, AI Copilot, Notifications & User Profile */}
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          {/* Mobile Search Trigger Button */}
          <button
            type="button"
            onClick={() => setMobileSearchOpen(!mobileSearchOpen)}
            className="md:hidden flex h-10 w-10 items-center justify-center rounded-xl text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-colors cursor-pointer"
            aria-label="Open search"
          >
            {mobileSearchOpen ? <X className="h-4 w-4" /> : <Search className="h-4 w-4" />}
          </button>

          {/* Quick AI Copilot Shortcut */}
          <Link
            href="/user/chat"
            className="flex items-center gap-1.5 min-h-[38px] px-2.5 sm:px-3 py-1.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold border border-blue-200/80 transition-colors shadow-2xs"
