      ...
      <main className="flex-1 py-8 px-4 sm:px-6 lg:px-10 overflow-y-auto max-w-5xl mx-auto w-full">
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-slate-200/90 bg-white p-6 sm:p-8 shadow-xs space-y-6">
            {error && (
              <div className="p-4 rounded-2xl bg-red-50 border border-red-200 text-xs font-bold text-red-700 flex items-center gap-2.5">
            ...
            <form onSubmit={handleUploadAndAnalyze} className="space-y-4">
              {inputMode === 'upload' ? (
                <div className="rounded-3xl border-2 border-dashed border-slate-300 p-8 text-center bg-slate-50/50 hover:bg-blue-50/20 hover:border-blue-300 transition-all space-y-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full rounded-2xl border border-slate-300 bg-white p-4 text-xs text-slate-900 focus:border-blue-600 focus:outline-none"
                  />
                </div>
              )}
              ...
              <div className="flex justify-end pt-2">
                <button
                  type="submit"
                  disabled={analyzing}
                  className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3 text-xs font-extrabold text-white shadow-sm shadow-blue-500/20 hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 transition-all hover:-translate-y-0.5"
                >
          ...
          {analysis && (
            <div className="rounded-3xl border border-slate-200/90 bg-white p-6 sm:p-10 shadow-xs space-y-8">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                ...
                <div className="flex items-center gap-4 bg-slate-50 p-3 rounded-2xl border border-slate-200/70">
                  ...
                  <span className={`px-2.5 py-1 rounded-xl text-xs font-bold border ${scoreTier.color}`}>
                    {scoreTier.label}
                  </span>
                </div>
              </div>
              ...
              {/* Executive Summary */}
              {analysis.summary && (
                <div className="rounded-2xl border border-blue-100 bg-blue-50/40 p-5 text-xs sm:text-sm text-slate-700 leading-relaxed space-y-1">
                ...
                <div className="rounded-2xl border border-emerald-100 bg-emerald-50/20 p-5 space-y-3">
                  <div className="flex items-center justify-between text-xs font-bold text-emerald-800">
                    <span className="flex items-center gap-1.5">
                      ...
                      Extracted Verified Skills ({analysis.extractedSkills?.length || 0})
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {(analysis.extractedSkills || []).map((s: string) => (
                      <span
                        key={s}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white border border-emerald-200 text-xs font-semibold text-slate-800 shadow-2xs"
                      >
                ...
                <div className="rounded-2xl border border-amber-100 bg-amber-50/20 p-5 space-y-3">
                  <div className="flex items-center justify-between text-xs font-bold text-amber-800">
                    <span className="flex items-center gap-1.5">
                      ...
                      Recommended High-Frequency Keywords ({analysis.suggestedKeywords?.length || 0})
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {(analysis.suggestedKeywords || []).map((kw: string) => (
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white border border-amber-200 text-xs font-bold text-blue-700 shadow-2xs hover:bg-blue-50 transition-colors"
                    ...
                    {analysis.weakBulletPoints.map((bp: any, idx: number) => (
                      ...
                      >
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="text-slate-900 font-bold pl-3 border-l-2 border-emerald-500 bg-emerald-50/40 p-2 rounded-r-xl">
